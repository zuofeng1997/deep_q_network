...
def run(double=False):
    ...
    writer = SummaryWriter(comment="n_steps_dqn")
    ...
    while True:
        ...
        while True:
            ...
            if done:
                ...
                a_ = choose_action(env=env, state=s_, net=net, epsilon=epsilon, use_cuda=args.cuda)
            ...
            if len(buffer) >= params["replay_start_size"]:
                ...
                learn(loss_fn, optimizer, batch, params, net, target_net, use_cuda=args.cuda)
