...
def run(double=False, dueling=False):
    ...
    net = dqn_model.DQN(env.observation_space.shape, env.action_space.n, duleing=dueling).to(device)
    target_net = dqn_model.DQN(env.observation_space.shape, env.action_space.n, duleing=dueling).to(device)
